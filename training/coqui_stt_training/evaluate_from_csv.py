...
def evaluate_from_csv(transcriptions_csv, ground_truth_csv, audio_path_trans_column="wav_filename", audio_path_gt_column="wav_filename", text_trans_column="transcript", transcription_gt_column="transcript"):
    # load csvs
    ...
    if len(df_transcriptions.values.tolist()) != len(df_gt.values.tolist()):
        return "ERROR: The following audios are missing in your CSV file: "+ str(set(df_gt[audio_path_trans_column].values.tolist()) - set(df_transcriptions[audio_path_trans_column].values.tolist()))

    # dropall except the audio and text key for transcription df 
    df_transcriptions = df_transcriptions.filter([audio_path_trans_column, text_trans_column])
